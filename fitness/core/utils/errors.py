...
class ErrorsFormatter:
    ...
    def __call__(self):
        if hasattr(self.exception, "get_full_details"):
            if isinstance(self.exception.get_full_details(), dict):
                try:
    ...
    def _get_response_json_from_error_message(
        ...
        response_data = {self.ERRORS: [{self.MESSAGE: message}]}

        if field:
            response_data[self.ERRORS][self.FIELD] = field
        if response_data.get(self.CODE):
            response_data.pop(self.CODE)
